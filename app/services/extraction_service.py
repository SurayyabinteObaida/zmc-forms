...
def _fix_year(data: dict) -> dict:
    """Forces date fields to year 2026."""
    if not data:
        return {}
    for key in ["date", "print_date"]:
        val = data.get(key)
        if val and isinstance(val, str):
            # Capture DD/MM or DD-MM. Ignore existing year if any.
            m = re.match(r"(\d{1,2})[-/](\d{1,2})", val)
            if m:
                # Force 2026
                data[key] = f"{m.group(1).zfill(2)}/{m.group(2).zfill(2)}/2026"
    return data


def _apply_flexo_rules(data: dict, form_code: str) -> dict:
    """Enforces Flexo job code prefix F-2602."""
    if form_code != "F-PRD/01.2":
        return data
    ...
    if job_code:
        jc_str = str(job_code).strip()
        # If job code already has the correct prefix, do nothing.
        if jc_str.startswith(prefix):
            return data

        # Otherwise, rebuild it. Assume the actual unique part is the last segment.
        # This handles cases like "123", "F-123", "F-2603-123", "2603-123"
        core_part = re.split(r'[- ]', jc_str)[-1]
        data["job_code"] = f"{prefix}-{core_part}"
...
def process_image(image_path: str, form_types: list) -> dict:
    try:
        ...
        # Fix year and flexo job code
        extracted["consensus"] = _fix_year(extracted["consensus"])
        extracted["consensus"] = _apply_flexo_rules(extracted["consensus"], form_code)
